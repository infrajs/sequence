...
class Seq {
	...
	public static function &add(&$obj, $right, &$val)
	{
		...
		$need = &Seq::_get($obj, $right, 0, $i, true);
	...
	public static function &set(&$obj, $right, &$val)
	{
		...
		$need = &Seq::_get($obj, $right, 0, $i, $make);
	...
	public static function &get($obj, $short, $def = null)
	{
		$right = Seq::right($short);
		$res = &Seq::_get($obj, $right);

		if (is_null($res)) return  $def;
		return $res;
	}
	public static function &_get(&$obj, $right, $start = 0, $end = null, $make = false)
	{
		...
		if (is_array($obj)) {
			...
			if ($make || (isset($obj[$right[$start]]) && !is_null($obj[$right[$start]]))) {
				...
				return Seq::_get($obj[$right[$start]], $right, ++$start, $end, $make);
			}
			...
			if (property_exists($obj, $name)) {
				...
				return Seq::_get($obj->$name, $right, ++$start, $end, $make);
			}
		} else {
			return $r;
		}
